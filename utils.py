...
async def aget_embeddings(text: List):
    """
    Creates embeddings for the raw text fetched by the scraper from the web.
...
async def get_embeddings(text: List):
    loop = asyncio.get_event_loop()
    embeddings = await loop.run_in_executor(None, aget_embeddings, text)
    return embeddings

async def amake_context(query: str, context: List, context_percentage: float):
    """
    Prepares the context for the LLM to answer the user query
    ...
    query_embedding_vector = query_embedding[0]['embedding']
    context_embeddings_vectors = [embedding['embedding'] for embedding in context_embeddings]

    similarities = cos_sim(query_embedding_vector, query_embedding_vector)
